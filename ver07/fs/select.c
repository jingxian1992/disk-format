...
int do_select(unsigned int in, unsigned int out, unsigned int ex,
	unsigned int *inp, unsigned int *outp, unsigned int *exp)
{
	int count;
...
/* Perform the select(nd, in, out, ex, tv) system call. */
	int i;
	unsigned int res_in, in = 0, *inp;
	unsigned int res_out, out = 0, *outp;
	unsigned int res_ex, ex = 0, *exp;
	unsigned int mask;
	struct timeval *tvp;
	unsigned int timeout;

	mask = ~((~0) << get_fs_long(buffer++));
	inp = (unsigned int *) get_fs_long(buffer++);
	outp = (unsigned int *) get_fs_long(buffer++);
	exp = (unsigned int *) get_fs_long(buffer++);
