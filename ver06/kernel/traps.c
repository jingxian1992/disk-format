...
char get_seg_byte(int seg, char * addr)
{
	char __res;
...
unsigned int get_seg_long(int seg, int * addr)
{
	unsigned int __res;
...
static void die(char * str,int esp_ptr,int nr)
{
	...
	int i;
	int real_CS_RPL;
	unsigned int __SS_val, __esp_val;
	...
	if (real_CS_RPL)
	{
		printk("\tthe process that cause this interrupt is in the user mode.\n");
		printk("\tSo we need to know CS, EIP, EFLAGS, SS and ESP.\n");
		printk("\tCS:  %04X\n", esp[1]);
		printk("\tEIP:  %p\n", esp[0]);
		printk("\tEFLAGS:  %p\n", esp[2]);
		printk("\tSS:  %04X\n", esp[4]);
	...
	{
		printk("\tthe process that cause this interrupt is in the kernel mode.\n");
		printk("\tSo we need to know CS, EIP, and EFLAGS.\n");
		printk("\tCS:  %04X\n", esp[1]);
		printk("\tEIP:  %p\n", esp[0]);
		printk("\tEFLAGS:  %p\n\n", esp[2]);
	}
...
void do_parallel_interrupt(int esp, int error_code)
{
	die("general protection", esp, error_code);
}
