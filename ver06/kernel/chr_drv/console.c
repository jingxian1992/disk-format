...
struct con_struct vc_cons [MAX_CONSOLES];
...
static inline unsigned short get_init_pos()
{
	unsigned short tmp;
...
void con_init(void)
{
	...
	display_ptr = ((char *)video_mem_base) + video_size_row * 3 - 90;
	while (*display_desc)
	{
...
//	count = sizeof(struct con_struct);
	for (currcons = 1; currcons < NR_CONSOLES; currcons++)
	{
//		for (i=0; i<count; i++)
//		{
//			*((char *)(vc_cons + currcons) + i) = *((char *)vc_cons + i);
//		}
